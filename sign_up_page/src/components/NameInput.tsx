...
export default function NameInput({ setValidName }: NameInputProps) {
  const [displayName, setDisplayName] = useState("");
  const [touchedColor, setTouchedColor] = useState("border-[#70334c2E]");

  // Keep parent in sync with local validation
  useEffect(() => {
    setValidName(displayName.trim().length > 0);
  }, [displayName, setValidName]);
